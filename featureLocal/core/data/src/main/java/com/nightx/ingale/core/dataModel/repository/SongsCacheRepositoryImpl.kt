...
class SongsCacheRepositoryImpl(
    private val songsCacheDao: SongsCacheDao,
    ...
    override suspend fun isCached(): Boolean {
        return songsCacheDao.observeSongsCache()
            .first().isEmpty().not()
    }
    ...
    private suspend fun saveLocally() = withContext(dispatchers.io) {
        ...
        if ((cursor?.count ?: -1) > 0) {
            ...
            cursor.close()
            songsCacheDao.replaceAll(localSongs)
        }
    ...
    private companion object {
        private const val MaxImageSizeByte = 1024 * 1024 // 1 MB
